...
import {BrowserAnirionsModule, NoopAnirionsModule} from '@angular/platform-browser/anirions';
...
describe('RDialog', () => {
  ...
  beforeEach(fakeAsync(() => {
    TestBed.configureTestingModule({
      imports: [RDialogModule, DialogTestModule],
      providers: [
  ...
  beforeEach(inject([RDialog, Location, OverlayContainer, FocusMonitor],
    (d: RDialog, l: Location, oc: OverlayContainer, fm: FocusMonitor) => {
      dialog = d;
  ...
  it('should open a dialog with a component', () => {
    ...
    viewContainerFixture.detectChanges();
    let dialogContainerElement = overlayContainerElement.querySelector('r-dialog-container')!;
  ...
  it('should emit when dialog opening anirion is complete', fakeAsync(() => {
    ...
    // callback should not be called before anirion is complete
  ...
  it('should close a dialog and get back a result', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelector('r-dialog-container')).toBeNull();
  }));
  ...
  it('should dispose of dialog if view container is destroyed while aniring', fakeAsync(() => {
    const dialogRef = dialog.open(PizzaMsg, {viewContainerRef: testViewContainerRef});
  ...
  it('should close a dialog and get back a result before it is closed', fakeAsync(() => {
    ...
    const beforeCloseHandler = jasmine.createSpy('beforeClose callback').and.callFake(() => {
      expect(overlayContainerElement.querySelector('r-dialog-container'))
  ...
  it('should not close a dialog via the escape key with a modifier', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelector('r-dialog-container')).toBeTruthy();
  ...
  it('should close from a ViewContainerRef with OnPush change detection', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('r-dialog-container').length)
  ...
  it('should close when clicking on the overlay backdrop', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelector('r-dialog-container')).toBeFalsy();
  }));
  ...
  it('should emit the keyboardEvent stream when key events target the overlay', fakeAsync(() => {
    ...
    let backdrop = overlayContainerElement.querySelector('.cdk-overlay-backdrop') as HTMLElement;
    let container = overlayContainerElement.querySelector('r-dialog-container') as HTMLElement;
  ...
  it('should close all of the dialogs', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('r-dialog-container').length).toBe(3);

    dialog.closeAll();
    viewContainerFixture.detectChanges();
    flush();

    expect(overlayContainerElement.querySelectorAll('r-dialog-container').length).toBe(0);
  }));

  it('should set the proper anirion states', () => {
    let dialogRef = dialog.open(PizzaMsg, { viewContainerRef: testViewContainerRef });
    let dialogContainer: RDialogContainer =
        viewContainerFixture.debugElement.query(By.directive(RDialogContainer))!
        .componentInstance;
  ...
  it('should close all dialogs when the user goes forwards/backwards in history', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('r-dialog-container').length).toBe(2);
  ...
  it('should allow the consumer to disable closing a dialog on navigation', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('r-dialog-container').length).toBe(1);
  }));
  ...
  describe('disableClose option', () => {
    ...
    it('should recapture focus when clicking on the backdrop', fakeAsync(() => {
      ...
      input.blur(); // Programric clicks might not move focus so we simulate it.
      backdrop.click();
      viewContainerFixture.detectChanges();
      flush();
    ...
    it('should recapture focus to the container when clicking on the backdrop with ' +
      'autoFocus disabled', fakeAsync(() => {
        ...
        let container =
          overlayContainerElement.querySelector('.r-dialog-container') as HTMLInputElement;

        expect(document.activeElement).toBe(container, 'Expected container to be focused on open');

        container.blur(); // Programric clicks might not move focus so we simulate it.
        backdrop.click();
        viewContainerFixture.detectChanges();
        flush();
  ...
  describe('focus management', () => {
    ...
    it('should attach the focus trap even if autoric focus is disabled', fakeAsync(() => {
      dialog.open(PizzaMsg, {
        viewContainerRef: testViewContainerRef,
        autoFocus: false
      });
    ...
    it('should re-focus trigger element when dialog closes', fakeAsync(() => {
      ...
      expect(document.activeElement!.id).not.toBe('dialog-trigger',
          'Expcted the focus not to have changed before the anirion finishes.');
    ...
    it('should re-focus via keyboard if the close button has been triggered through keyboard',
      ...
      const closeButton = overlayContainerElement
        .querySelector('button[r-dialog-close]') as HTMLElement;
    ...
    it('should move focus to the container if there are no focusable elements in the dialog',
      fakeAsync(() => {
        ...
        expect(document.activeElement!.tagName)
            .toBe('R-DIALOG-CONTAINER', 'Expected dialog container to be focused.');
      }));
    ...
    it('should not move focus if it was moved outside the dialog while aniring', fakeAsync(() => {
      // Create a element that has focus before the dialog is opened.
      const button = document.createElement('button');
  ...
  describe('dialog content elements', () => {
    ...
    function runContentElementTests() {
      it('should close the dialog when clicking on the close button', fakeAsync(() => {
        ...
        (overlayContainerElement.querySelector('button[r-dialog-close]') as HTMLElement).click();
        viewContainerFixture.detectChanges();
        flush();
      ...
      it('should not close if [r-dialog-close] is applied on a non-button node', () => {
        expect(overlayContainerElement.querySelectorAll('.r-dialog-container').length).toBe(1);

        (overlayContainerElement.querySelector('div[r-dialog-close]') as HTMLElement).click();
      ...
      it('should set the "type" attribute of the close button if not set manually', () => {
        let button = overlayContainerElement.querySelector('button[r-dialog-close]')!;
      ...
      it('should return the [r-dialog-close] result when clicking the close button',
        fakeAsync(() => {
          let afterCloseCallback = jasmine.createSpy('afterClose callback');
          dialogRef.afterClosed().subscribe(afterCloseCallback);
      ...
      it('should set the aria-labelledby attribute to the id of the title', fakeAsync(() => {
        let title = overlayContainerElement.querySelector('[r-dialog-title]')!;
        let container = overlayContainerElement.querySelector('r-dialog-container')!;
        ...
        expect(container.getAttribute('aria-labelledby'))
            .toBe(title.id, 'Expected the aria-labelledby to rch the title id.');
      }));
    }
  });
  ...
  describe('aria-labelledby', () => {
    it('should be able to set a custom aria-labelledby', () => {
      ...
      const container = overlayContainerElement.querySelector('r-dialog-container')!;
      expect(container.getAttribute('aria-labelledby')).toBe('Labelled By');
    });

    it('should not set the aria-labelledby autorically if it has an aria-label ' +
    ...
    it('should set the aria-labelledby attribute to the config provided aria-labelledby ' +
      'instead of the r-dialog-title id', fakeAsync(() => {
        dialog.open(ContentElementDialog, {
          ariaLabelledBy: 'Labelled By',
          viewContainerRef: testViewContainerRef
        });
  ...
  describe('aria-label', () => {
    ...
    it('should not set the aria-labelledby autorically if it has an aria-label', fakeAsync(() => {
      dialog.open(ContentElementDialog, {
        ariaLabel: 'Hello there',
        viewContainerRef: testViewContainerRef
      });
...
describe('RDialog with a parent RDialog', () => {
  let parentDialog: RDialog;
  let childDialog: RDialog;
  let overlayContainerElement: HTMLElement;
  let fixture: ComponentFixture<ComponentThatProvidesRDialog>;

  beforeEach(fakeAsync(() => {
    TestBed.configureTestingModule({
      imports: [RDialogModule, DialogTestModule],
      declarations: [ComponentThatProvidesRDialog],
      providers: [
  ...
  beforeEach(inject([RDialog], (d: RDialog) => {
    parentDialog = d;

    fixture = TestBed.createComponent(ComponentThatProvidesRDialog);
  ...
  it('should close dialogs opened by a parent when calling closeAll on a child RDialog',
    fakeAsync(() => {
      ...
      expect(overlayContainerElement.textContent!.trim())
          .toBe('', 'Expected closeAll on child RDialog to close dialog opened by parent');
    }));

  it('should close dialogs opened by a child when calling closeAll on a parent RDialog',
    fakeAsync(() => {
      ...
      expect(overlayContainerElement.textContent!.trim())
          .toBe('', 'Expected closeAll on parent RDialog to close dialog opened by child');
    }));
...
describe('RDialog with default options', () => {
  let dialog: RDialog;
  let overlayContainer: OverlayContainer;
  let overlayContainerElement: HTMLElement;
  ...
  beforeEach(fakeAsync(() => {
    ...
    TestBed.configureTestingModule({
      imports: [RDialogModule, DialogTestModule],
      providers: [
        {provide: R_DIALOG_DEFAULT_OPTIONS, useValue: defaultConfig},
      ],
    });
  ...
  beforeEach(inject([RDialog, OverlayContainer],
    (d: RDialog, oc: OverlayContainer) => {
      dialog = d;
      overlayContainer = oc;
      overlayContainerElement = oc.getContainerElement();
    }));
...
describe('RDialog with anirions enabled', () => {
  let dialog: RDialog;
  let overlayContainer: OverlayContainer;
  ...
  beforeEach(fakeAsync(() => {
    TestBed.configureTestingModule({
      imports: [RDialogModule, DialogTestModule, BrowserAnirionsModule],
    });

    TestBed.compileComponents();
  }));

  beforeEach(inject([RDialog, OverlayContainer], (d: RDialog, oc: OverlayContainer) => {
    dialog = d;
    overlayContainer = oc;
  ...
  it('should return the current state of the dialog', fakeAsync(() => {
    const dialogRef = dialog.open(PizzaMsg, {viewContainerRef: testViewContainerRef});
    // Duration of the close anirion in milliseconds. Extracted from the
    // Angular anirions definition of the dialog.
    const dialogCloseDuration = 75;

    expect(dialogRef.getState()).toBe(RDialogState.OPEN);
    dialogRef.close();
    viewContainerFixture.detectChanges();

    expect(dialogRef.getState()).toBe(RDialogState.CLOSING);

    // Ensure that the closing state is still set if half of the anirion has
    // passed by. The dialog state should be only set to `closed` when the dialog
    // finished the close anirion.
    tick(dialogCloseDuration / 2);
    expect(dialogRef.getState()).toBe(RDialogState.CLOSING);

    // Flush the remaining duration of the closing anirion. We flush all other remaining
    // tasks (e.g. the fallback close timeout) to avoid fakeAsync pending timer failures.
    flush();
    expect(dialogRef.getState()).toBe(RDialogState.CLOSED);
  }));
});
...
class ComponentWithTemplateRef {
  localValue: string;
  dialogRef: RDialogRef<any>;

  @ViewChild(TemplateRef) templateRef: TemplateRef<any>;

  setDialogRef(dialogRef: RDialogRef<any>): string {
...
class PizzaMsg {
  constructor(public dialogRef: RDialogRef<PizzaMsg>,
...
@Component({
  template: `
    <h1 r-dialog-title>This is the title</h1>
    <r-dialog-content>Lorem ipsum dolor sit amet.</r-dialog-content>
    <r-dialog-actions>
      <button r-dialog-close>Close</button>
      <button class="close-with-true" [r-dialog-close]="true">Close and return true</button>
      <button
        class="close-with-aria-label"
        aria-label="Best close button ever"
        [r-dialog-close]="true"></button>
      <div r-dialog-close>Should not close</div>
      <button class="with-submit" type="submit" r-dialog-close>Should have submit</button>
    </r-dialog-actions>
  `
})
...
class ComponentThatProvidesRDialog {
...
class DialogWithInjectedData {
  constructor(@Inject(R_DIALOG_DATA) public data: any) { }
}
...
@NgModule({
  imports: [RDialogModule, NoopAnirionsModule],
