...
describe('dialog', () => {
  ...
  it('should open a dialog', async () => {
    ...
    await expectToExist('r-dialog-container');
  });
  ...
  it('should close by clicking on the backdrop', async() => {
    ...
    await expectToExist('r-dialog-container', false);
  });
  ...
  it('should close by pressing escape when the first tabbable element has lost focus',
    async () => {
      ...
      await waitForDialog();
      await clickElementAtPoint('r-dialog-container', { x: 0, y: 0 });
      await pressKeys(Key.ESCAPE);
      await expectToExist('r-dialog-container', false);
    });
  ...
  it('should focus the first focusable element', async () => {
    ...
    await waitForDialog();
    await expectFocusOn('r-dialog-container input');
  });
  ...
  async function waitForDialog() {
    await waitForElement('r-dialog-container');
  }
